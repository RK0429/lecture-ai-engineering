...
def load_model():
    ...
    try:
        ...
        # アクセストークンを保存し、Hugging Faceにログイン
...
def generate_response(
    pipe, user_question, max_new_tokens=512, temperature=0.7, top_p=0.9
):
    """LLMを使用して質問に対する回答を生成する。生成パラメータを引数で受け取る"""
    if pipe is None:
    ...
    try:
        ...
        if outputs and isinstance(outputs, list) and outputs[0].get("generated_text"):
            if (
                isinstance(outputs[0]["generated_text"], list)
                and len(outputs[0]["generated_text"]) > 0
            ):
                # messages形式の場合
                ...
                if "<start_of_turn>model" in possible_response:
                    assistant_response = possible_response.split(
                        "<start_of_turn>model\n"
                    )[-1].strip()
                else:
        ...
        if not assistant_response:
            ...
            print(
                "Warning: Could not extract assistant response. Full output:", outputs
            )
