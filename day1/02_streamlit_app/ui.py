...
def display_chat_page(pipe):
    """チャットページのUIを表示する"""
    # --- LLM生成パラメータ設定 ---
    with st.sidebar.expander("LLM設定"):  # サイドバーにパラメータUI
        ...
        top_p = st.slider(
            "Top-p", min_value=0.0, max_value=1.0, value=0.9, step=0.01, key="top_p"
        )
    ...
    if submit_button and user_question:
        ...
        with st.spinner("モデルが回答を生成中..."):
            answer, response_time = generate_response(
                pipe, user_question, max_new_tokens, temperature, top_p
            )
...
def display_history_list(history_df):
    ...
    search_query = st.text_input(
        "📝 キーワード検索 (質問または回答)", value="", key="history_search"
    )
    # 表示オプション
    filter_options = {
        "すべて表示": None,
    ...
    # ソートオプション
    sort_options = {
        "日時 降順": ("timestamp", False),
        "日時 昇順": ("timestamp", True),
        "応答時間 降順": ("response_time", False),
        "応答時間 昇順": ("response_time", True),
        "精度スコア 降順": ("is_correct", False),
        "精度スコア 昇順": ("is_correct", True),
    }
    sort_choice = st.selectbox(
        "🔃 並び替え", options=list(sort_options.keys()), key="history_sort"
    )
